...
static void print_usage(const char *program_name) {
    fprintf(stderr, "Uso:\n");
    fprintf(stderr, "  %s <imagen.pgm de cualquier resolucion> [counter]\n", program_name);
    fprintf(stderr, "\nEjemplo futuro con OpenMPI:\n");
    fprintf(stderr, "  mpirun -np 1 %s images/cuadrado.pgm : -np 1 ./geoboard_server : -np 3 ./geoboard_worker\n", program_name);
    fprintf(stderr, "\nNotas:\n");
    fprintf(stderr, "  - El cliente no exige un tamano fijo de imagen.\n");
    fprintf(stderr, "  - La adaptacion a 3x3 y a mascara 8x8 la realiza el servidor.\n");
...
int client_rank_main(int argc, char **argv, int world_size) {
    ...
    printf("[CLIENTE] El tamano/resolucion no se valida aqui; el servidor adapta la imagen.\n");
